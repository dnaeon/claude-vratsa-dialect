...
Името след `@` е името на маркетплейсо (`vratsa-dialect`), а туй преди `@` е името на плъгина (пак `vratsa-dialect`, така уйдиса). Версия има, ъпдейти стават с `/plugin update`. Туй е жик так за всеки нормален пустиняк.
